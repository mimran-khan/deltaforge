...
def generate_html(trades: list[dict], events: list[dict], capital: dict) -> str:
    ...
    total_deployed = starting_cap + sum(
        inj.get("amount", 0) for inj in capital.get("capital_injections", [])
    )
