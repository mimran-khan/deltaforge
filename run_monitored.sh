...
# Post-session: regenerate GitHub Pages dashboard and push
DEV="${DELTAFORGE_DEV_REPO:-$HOME/Documents/TradingAgent}"
if [ -d "$DEV/reports" ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S %Z')] Regenerating dashboard report..."
    (cd "$DEV" && python -m reports.generate) 2>&1 && {
        echo "[$(date '+%Y-%m-%d %H:%M:%S %Z')] Pushing dashboard update to GitHub..."
        cd "$DEV"
        git add docs/index.html
        git commit -m "chore: update dashboard $(date '+%Y-%m-%d')" --no-verify 2>/dev/null
        git push origin main 2>&1 || echo "[$(date '+%Y-%m-%d %H:%M:%S %Z')] WARNING: git push failed"
        cd "$OLDPWD"
    } || echo "[$(date '+%Y-%m-%d %H:%M:%S %Z')] WARNING: report generation failed"
fi
